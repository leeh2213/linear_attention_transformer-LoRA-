    ...
    def init_lora_param(self):
        for param_name, lora_name in self.params_with_lora.items():
            if hasattr(self, f'{lora_name}_lora_A'):
                ...
                print(f'Initializing {lora_name}')
...
class LoRASelfAttention(nn.Module, LoRALayer):
    def __init__(
        self,
        dim,
        heads,
        ...
        self.heads = heads
        self.d_heads = default(dim_head, dim // heads)
        ...
        # base projections
        self.to_q = nn.Linear(dim, heads * self.d_heads, bias=False)
        self.to_k = nn.Linear(dim, heads * self.d_heads, bias=False)
        self.to_v = nn.Linear(dim, heads * self.d_heads, bias=False)
        self.to_out = nn.Linear(heads * self.d_heads, dim)

        # register LoRA on specified modules
        if self.r > 0:
            if 'q' in enable_lora:
                self.params_with_lora.update({'to_q.weight': 'q'})
            if 'k' in enable_lora:
                self.params_with_lora.update({'to_k.weight': 'k'})
            if 'v' in enable_lora:
                self.params_with_lora.update({'to_v.weight': 'v'})
            if 'o' in enable_lora:
                self.params_with_lora.update({'to_out.weight': 'o'})
    ...
    def forward(self, x, input_mask=None, **kwargs):
        ...
        b, t, _ = q.shape
        # reshape into heads
        q = q.view(b, t, self.heads, self.d_heads).transpose(1, 2)
        k = k.view(b, t, self.heads, self.d_heads).transpose(1, 2)
        v = v.view(b, t, self.heads, self.d_heads).transpose(1, 2)

        # split local vs global
        lq, qg = split_at_index(1, self.local_attn_heads, q)
        lk, kg = split_at_index(1, self.local_attn_heads, k)
        lv, vg = split_at_index(1, self.local_attn_heads, v)

        out_chunks = []
        if self.local_attn_heads > 0:
            local_out = self.local_attn(lq, lk, lv, input_mask=input_mask)
            out_chunks.append(local_out)
        if self.global_attn_heads > 0:
            global_out = self.global_attn_fn(qg, kg, vg, kv_mask=input_mask)
            out_chunks.append(global_out)
        ...
        # subtract LoRA in eval mode to keep base weights clean
        if self.r > 0 and not self.training:
            self.sub_lora_data()
    ...
    def __init__(
        ...
        for ind, local_heads in zip(range(depth), n_local_attn_heads):
            ...
            if not exists(linformer_settings):
                # attn = LoRASelfAttention(dim, heads, causal, dim_head = dim_head, blindspot_size = blindspot_size, n_local_attn_heads = local_heads, local_attn_window_size = local_attn_window_size, dropout = attn_layer_dropout, attn_dropout= attn_dropout)
                attn = LoRASelfAttention(dim, heads, r=8, lora_alpha=16, enable_lora=['q','k','v','o'], causal=causal, dim_head=dim_head, blindspot_size=blindspot_size, n_local_attn_heads=local_heads, local_attn_window_size=local_attn_window_size, attn_dropout=attn_dropout, dropout=attn_layer_dropout)
            else:
            ...
            if attend_axially:
                layers.append(nn.ModuleList([
                    PreNorm(dim, FoldAxially(local_attn_window_size, LoRASelfAttention(dim, heads, r=8, lora_alpha=16, enable_lora=['q','k','v','o'], causal = causal, attn_dropout = attn_dropout, dropout = attn_layer_dropout))),
                    PreNorm(dim, Chunk(ff_chunks, FeedForward(dim, glu = ff_glu, dropout= ff_dropout), along_dim = 1))
                ]))
                # LoRASelfAttention(dim, heads, causal, dropout = attn_layer_dropout, attn_dropout= attn_dropout))
                
                

            if receives_context:
                if not exists(context_linformer_settings):
                    attn = LoRASelfAttention(dim, heads, r=8, lora_alpha=16, enable_lora=['q','k','v','o'], dim_head = dim_head, dropout = attn_layer_dropout, attn_dropout= attn_dropout, receives_context = True)
                else:
